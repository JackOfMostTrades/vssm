...
func GetPeers(myRegion string, myAsg string) ([]string, error) {
	if myAsg == "" || myRegion == "" {
		return nil, nil
	}

	sess, err := session.NewSession(aws.NewConfig().WithRegion(myRegion))
	if err != nil {
		return nil, err
	}
	...
	output, err := asg.DescribeAutoScalingGroups(&autoscaling.DescribeAutoScalingGroupsInput{
		AutoScalingGroupNames: []*string{&myAsg},
	})
	if err != nil {
		return nil, err
	}
